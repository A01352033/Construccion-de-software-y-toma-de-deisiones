-- Crear base de datos materialesdb
CREATE DATABASE manudb;
...
--==============================================================
-- debido al incremento en los precios, se solicita que genere una tabla con los precios actualizados de los materiales
-- en un 15% no se pide que actualice el valor original, se pide que muestre el incremento tentativo
--==============================================================
-- qeury

SELECT clave, descripcion, precio as 'Precio Original', precio*1.15 as 'Precio Actualizado' FROM materiales
ORDER BY precio;


--==============================================================
/* Descripcion del materiales y el total de unidades entregas (por cada material), solo de aquellos con entegas
    de unidades mayores a 1000 unidades, ordenados de mayor a menor */
--==============================================================
-- query

SELECT m.descripcion, SUM(e.cantidad) as 'Total de unidades entregadas'
...
--==============================================================
/* Razon social del proveedor que entregaron más unidades que el proveedor
    con razon social 'Comex' */
--==============================================================

-- query
SELECT p.razonsocial, SUM(e.cantidad) as 'Total de unidades entregadas' -- se obtienen los campos a mostrar
FROM proveedores as p, entregan as e -- se obtienen las tablas
WHERE p.rfc = e.rfc -- se relacionan las tablas
GROUP BY p.razonsocial -- se agrupa por razon social
HAVING SUM(e.cantidad) > (SELECT SUM(e.cantidad) FROM entregan as e, proveedores as p WHERE p.rfc = e.rfc AND p.razonsocial = 'Comex') -- se obtiene el total de unidades entregadas por el proveedor 'Comex'
ORDER BY SUM(e.cantidad) DESC; -- se ordena de mayor a menor